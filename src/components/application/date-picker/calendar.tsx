...
export const CalendarContextProvider = ({ children }: PropsWithChildren) => {
    ...
    // AriaCalendarContext's onChange also accepts a range (array) value for
    // range-calendar consumers - this provider only ever holds a single
    // DateValue, so an array just takes its first date.
    const onChange = (next: DateValue | readonly DateValue[]) => {
        setValue(Array.isArray(next) ? (next[0] ?? null) : (next as DateValue));
    };
