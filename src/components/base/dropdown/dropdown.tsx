...
const DropdownSeparator = (props: AriaSeparatorProps) => {
    // ponytail: AriaSeparator renders a real <hr>, and every other <hr>-based
    // divider in this kit (nav-list.tsx, section-divider.tsx) pairs
    // bg-border-secondary with border-none - without it, the browser's
    // default <hr> border (1px solid currentColor, i.e. dark text color)
    // rendered on top of the correct light-grey background fill, making the
    // whole thing look black. This was the one divider missing border-none.
    return <AriaSeparator {...props} className={cx("my-1 h-px w-full border-none bg-border-secondary", props.className)} />;
};
